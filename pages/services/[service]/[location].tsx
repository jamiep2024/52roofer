...
export const getStaticProps: GetStaticProps<ServicePageProps> = async ({ params }) => {
  if (!params?.service || !params?.location || typeof params.service !== 'string' || typeof params.location !== 'string') {
  ...
  const parseLocationSlug = (slug: string) => {
    if (slug.endsWith('-oxford')) {
      return {
        location: slug.replace('-oxford', '').split('-').map(word => 
    ...
    if (slug.endsWith('-wiltshire')) {
      return {
        location: slug.replace('-wiltshire', '').split('-').map(word => 
          word.charAt(0).toUpperCase() + word.slice(1)
        ).join(' '),
        county: 'Wiltshire'
      };
