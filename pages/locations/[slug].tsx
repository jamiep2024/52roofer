...
export default function LocationPage({ location, slug }: LocationPageProps) {
  ...
  const generatedDescription = `We provide professional roofing services in ${location.name}, ${location.county}. ${
    location.roofingInfo?.commonStyles ? `With expertise in ${location.roofingInfo.commonStyles.join(', ')}, ` : ''
  }our team specializes in addressing local challenges${
    location.roofingInfo?.challenges ? ` such as ${location.roofingInfo.challenges.join(', ')}` : ''
  }. We understand the unique requirements of ${location.name}'s buildings${
    location.heritage?.listedBuildings?.gradeII?.length ? `, including ${location.heritage.listedBuildings.gradeII.length} Grade II listed properties` : ''
  }${
    location.roofingInfo?.buildingRegulations?.planningRequirements ? `, and work in compliance with local ${location.roofingInfo.buildingRegulations.planningRequirements}` : ''
  }. Our services are tailored to handle${
    location.roofingInfo?.weatherConsiderations?.commonIssues ? ` ${location.roofingInfo.weatherConsiderations.commonIssues.join(', ')} common in the area` : ' all types of roofing needs'
  }.`;
