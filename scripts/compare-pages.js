...
async function comparePages() {
    try {
        ...
        const missingFromSitemap = pageFiles.filter(page => !sitemapUrls.includes(page));
        console.log('\nPages in filesystem but not in sitemap:', missingFromSitemap.length);
        missingFromSitemap.sort().forEach(page => console.log(`- ${page}`));
        
        // Find URLs that are in sitemap but not in filesystem
        const missingFromFiles = sitemapUrls.filter(url => !pageFiles.includes(url));
        console.log('\nURLs in sitemap but not in filesystem:', missingFromFiles.length);
