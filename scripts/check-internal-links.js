...
function getAllPages(dir, baseDir = dir) {
    ...
    for (const item of items) {
        ...
        } else if (item.endsWith('.tsx') && !item.startsWith('_') && !item.startsWith('sitemap')) {
            pages.push(fullPath);
        }
    }
...
function analyzeInternalLinks() {
    ...
    const locationPages = Array.from(linkMap.keys()).filter(page => page.startsWith('locations/'));
    locationPages.forEach(page => {
        const county = page.split('/')[1];
        if (county && !linkMap.get(page).includes(`county/${county}`)) {
            console.log(`- ${page} should link to its county page (county/${county})`);
        }
    });
    
    // Check if service pages link to related blog posts
    const servicePages = Array.from(linkMap.keys()).filter(page => page.startsWith('services/'));
    const blogPosts = Array.from(linkMap.keys()).filter(page => page.startsWith('blog/'));
        ...
        if (relatedPosts.length > 0) {
            ...
            relatedPosts.forEach(post => {
                if (!links.includes(post)) {
                    console.log(`- ${servicePage} could link to related blog post: ${post}`);
                }
            });
        }
    });
    ...
    const rooferPages = Array.from(linkMap.keys()).filter(page => page.startsWith('roofers-in-'));
    rooferPages.forEach(page => {
        ...
        if (!linkMap.get(page).includes(mainLocation)) {
            console.log(`- ${page} should link to its main location page (${mainLocation})`);
        }
    });
}
